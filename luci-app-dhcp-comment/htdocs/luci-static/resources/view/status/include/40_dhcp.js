...
return baseclass.extend({
	...
	handleCreateStaticLease6(lease, ev) {
		...
		var duid_iaid = lease.duid.toLowerCase();
		if (lease.iaid)
	...
	renderLeases(dhcp_leases, host_hints, macaddr) {
		...
		uci.sections('dhcp', 'host').forEach(function(s) {
			L.toArray(s.mac).forEach(function(m) {
				if (s.comment) mac_comments[m.toLowerCase()] = s.comment;
			});
		});
		...
		cbi_update_table(table, leases.map(L.bind(function(lease) {
			...
			if (lease.macaddr && mac_comments[lease.macaddr.toLowerCase()]) {
				var cmt = mac_comments[lease.macaddr.toLowerCase()];
				var raw_h = lease.hostname || (hint ? hint[1] : null);
				host = raw_h ? (cmt + ' (' + raw_h + ')') : cmt;
			}
